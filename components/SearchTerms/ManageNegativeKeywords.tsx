...
const ManageNegativeKeywords = ({
  data,
  keywordLevel,
}: ManageNegativeKeywords) => {
  const [openBulkMatch, setOpenBulkMatchType] = useState<boolean>(false);
  const [rowSelection, setRowSelection] = useState({});
  const [openAddNegativeKeyword, setOpenAddNegativeKeyword] =
    useState<boolean>(false);
  const { bulkRemoveIsLoading, bulkRemoveNegativeKeywords } =
  ...
  const handleNewNegativeKeywordModal = () =>
    setOpenAddNegativeKeyword((prev) => !prev);

  const getUniqueAdGroups = useMemo(() => {
    const adGroupMap = new Map();
    data.forEach((group) => {
      if (!adGroupMap.has(group?.ad_group?.id)) {
        adGroupMap.set(group?.ad_group?.id, {
          name: group?.ad_group?.name,
          id: group?.ad_group?.id,
          campaign: group?.ad_group?.campaign,
        });
      }
    });
  ...
  const columns = useMemo<MRT_ColumnDef<NegativeKeywordView>[]>(
    () => [
      {
  ...
  const table = useMaterialReactTable?.({
    ...
    getRowId: (row, index) =>
      `${row?.ad_group?.id}-${row?.criterion_id}-${row?.keyword?.match_type}-${row?.keyword?.text}`,
    renderTopToolbar: ({ table }) => {
      ...
      const uniqueAdGroups = new Map();

      table.getRowModel().rows.forEach((row) => {
        const adGroup = row.original.ad_group;
        if (adGroup && !uniqueAdGroups.has(adGroup.id)) {
          uniqueAdGroups.set(adGroup.id, adGroup);
        }
      });

      // Convert the Map values back to an array for the modal
      const adGroupsForModal = Array.from(uniqueAdGroups.values());
      ...
      return (
        <>
          ...
          <AddNegativeKeywordsModal
            open={openAddNegativeKeyword}
            onClose={handleNewNegativeKeywordModal}
            adGroup={getUniqueAdGroups}
            keywordLevel={keywordLevel}
          />
          <Stack
            sx={{ padding: "1rem" }}
            ...
            spacing={2}
          >
            <Tooltip title="Add Ad Group Negative Keyword">
              <Fab
                size="small"
                color="primary"
                onClick={handleNewNegativeKeywordModal}
              >
                <AddIcon />
              </Fab>
            </Tooltip>
            ...
            {!noneSelected && (
              <>
                ...
                <Button
                  ...
                  color="error"
                  onClick={async () => {
                    await bulkRemoveNegativeKeywords(rowSelection).then(
                      async () => {
